...
namespace Box2D
{
        ...
        public void Reset(Contact[] contacts, int contactCount, float impulseRatio)
        {
            ...
            for (int i = 0; i < _constraintCount; ++i)
            {
                ...
                for (int j = 0; j < cc.pointCount; ++j)
                {
...
#if MATH_OVERLOADS
                    Vec2 tangent = MathUtils.Cross(cc.normal, 1.0f);

                    float rtA = MathUtils.Cross(ccp.rA, tangent);
                    float rtB = MathUtils.Cross(ccp.rB, tangent);
#else
                    Vec2 tangent = new Vec2(cc.normal.Y, -cc.normal.X);
                    ...
                    float vRel = Vec2.Dot(cc.normal, vB + MathUtils.Cross(wB, ccp.rB) - vA - MathUtils.Cross(wA, ccp.rA));
                ...
                if (cc.pointCount == 2)
                {
                    ...
                    {
                        // K is safe to invert.
                        cc.K = new Mat22(new Vec2(k11, k12), new Vec2(k12, k22));
        ...
        public void WarmStart()
        {
            ...
            for (int i = 0; i < _constraintCount; ++i)
            {
                ...
                for (int j = 0; j < c.pointCount; ++j)
                {
...
#else
                    Vec2 P = new Vec2(ccp.normalImpulse * normal.X + ccp.tangentImpulse * tangent.X,
                    ...
                    Debug.Assert(!(float.IsNaN(bodyB._linearVelocity.X) || float.IsNaN((bodyB._linearVelocity.Y))), "LinearVelocity is NaN");
#endif
                    c.points[j] = ccp;
                }
        ...
        public void SolveVelocityConstraints()
        {
            for (int i = 0; i < _constraintCount; ++i)
            {
...
#if MATH_OVERLOADS
                Vec2 tangent = ZoomEngine.Physics.Common.Math.Cross(normal, 1.0f);
#else
                Vec2 tangent = new Vec2(normal.Y, -normal.X);
#endif
                ...
                for (int j = 0; j < c.pointCount; ++j)
                {
                    ...
                    Vec2 dv = vB + MathUtils.Cross(wB, ccp.rB) - vA - MathUtils.Cross(wA, ccp.rA);

                    // Compute tangent force
                    float vt = Vec2.Dot(dv, tangent);
#else
                    // Relative velocity at contact
                    Vec2 dv = new Vec2(vB.X + (-wB * ccp.rB.Y) - vA.X - (-wA * ccp.rA.Y),
...
#else
                    // Apply contact impulse
                    Vec2 P = new Vec2(lambda * tangent.X, lambda * tangent.Y);
                ...
                if (c.pointCount == 1)
                {
                    ...
                    // Apply contact impulse
                    var P = new Vec2(lambda * normal.X, lambda * normal.Y);
                ...
                else
                {
...
#else
                    // Relative velocity at contact
                    Vec2 dv1 = new Vec2(vB.X + (-wB * cp1.rB.Y) - vA.X - (-wA * cp1.rA.Y),
                                              vB.Y + (wB * cp1.rB.X) - vA.Y - (wA * cp1.rA.X));
                    Vec2 dv2 = new Vec2(vB.X + (-wB * cp2.rB.Y) - vA.X - (-wA * cp2.rA.Y),
                    ...
                    while (true)
                    {
                        ...
                        // x' = - inv(A) * b'
                        //
                        Vec2 x = - MathUtils.Multiply(ref c.normalMass, b);
...
#else
                            // Resubstitute for the incremental impulse
                            Vec2 d = new Vec2(x.X - a.X, x.Y - a.Y);

                            // Apply incremental impulse
                            Vec2 P1 = new Vec2(d.X * normal.X, d.X * normal.Y);
                            Vec2 P2 = new Vec2(d.Y * normal.X, d.Y * normal.Y);
                            Vec2 P12 = new Vec2(P1.X + P2.X, P1.Y + P2.Y);
                ...
                Debug.Assert(!(float.IsNaN(bodyA._linearVelocity.X) || float.IsNaN((bodyA._linearVelocity.Y))), "LinearVelocity is NaN");
                Debug.Assert(!(float.IsNaN(bodyB._linearVelocity.X) || float.IsNaN((bodyB._linearVelocity.Y))), "LinearVelocity is NaN");
            }
        }
        ...
        public bool SolvePositionConstraints(float baumgarte)
        {
            ...
            for (int i = 0; i < _constraintCount; ++i)
            {
                ...
                for (int j = 0; j < c.pointCount; ++j)
                {
                    ...
                    Vec2 normal = psm._normal;
                    
                    Vec2 point = psm._point;
...
#else
                    Vec2 P = new Vec2(impulse * normal.X, impulse * normal.Y);
    ...
    internal struct PositionSolverManifold
    {
            ...
            case ManifoldType.Circles:
                {
                    ...
                    _separation = Vec2.Dot(pointB - pointA, _normal) - cc.radius;
                }
                break;
            ...
            case ManifoldType.FaceA:
                {
                    ...
                    Vec2 clipPoint = cc.bodyB.GetWorldPoint(cc.points[index].localPoint);
                    _separation = Vec2.Dot(clipPoint - planePoint, _normal) - cc.radius;
                    _point = clipPoint;
                }
                break;
            ...
            case ManifoldType.FaceB:
                {
                    ...
                    Vec2 clipPoint = cc.bodyA.GetWorldPoint(cc.points[index].localPoint);
                    _separation = Vec2.Dot(clipPoint - planePoint, _normal) - cc.radius;
                    _point = clipPoint;
        ...
        internal Vec2 _normal;
        internal Vec2 _point;
