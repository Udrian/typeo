...
namespace Box2D
{
    public class PolygonShape : Shape
    {
        ...
        static Vec2 ComputeCentroid(ref FixedArray8<Vec2> vs, int count)
        {
        ...
        {
            _vertexCount = 4;
            _vertices[0] = new Vec2(-hx, -hy);
            _vertices[1] = new Vec2( hx, -hy);
            _vertices[2] = new Vec2( hx,  hy);
            _vertices[3] = new Vec2(-hx,  hy);
            _normals[0] = new Vec2(0.0f, -1.0f);
            _normals[1] = new Vec2(1.0f, 0.0f);
            _normals[2] = new Vec2(0.0f, 1.0f);
            _normals[3] = new Vec2(-1.0f, 0.0f);
            _centroid = Vec2.Zero;
        }
        ...
        /// @see Shape.TestPoint
        public override bool TestPoint(ref Transform xf, Vec2 p)
        {
            Vec2 pLocal = MathUtils.MultiplyT(ref xf.R, p - xf.Position);
        ...
        public override bool RayCast(out RayCastOutput output, ref RayCastInput input, ref Transform xf, int childIndex)
        {
            ...
            Vec2 p1 = MathUtils.MultiplyT(ref xf.R, input.p1 - xf.Position);
            Vec2 p2 = MathUtils.MultiplyT(ref xf.R, input.p2 - xf.Position);
            Vec2 d = p2 - p1;
            ...
            if (_vertexCount == 2)
            {
                Vec2 v1 = _vertices[0];
                Vec2 v2 = _vertices[1];
                Vec2 normal = _normals[0];

                // q = p1 + t * d
                // dot(normal, q - v1) = 0
                // dot(normal, p1 - v1) + t * dot(normal, d) = 0
                float numerator = Vec2.Dot(normal, v1 - p1);
        ...
        {
            Vec2 lower = MathUtils.Multiply(ref xf, _vertices[0]);
            Vec2 upper = lower;

            for (int i = 1; i < _vertexCount; ++i)
            {
                Vec2 v = MathUtils.Multiply(ref xf, _vertices[i]);
        ...
        public Vec2 _centroid;
        public FixedArray8<Vec2> _vertices;
        public FixedArray8<Vec2> _normals;
