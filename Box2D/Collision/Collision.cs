...
namespace Box2D
{
    ...
    public struct Manifold
    {
        ...
        public Vec2 _localNormal;                        ///< not use for Type.SeparationFunction.Points
        public Vec2 _localPoint;                            ///< usage depends on manifold type
    ...
    public struct WorldManifold
    {
                        ...
                        ref Transform xfB, float radiusB)
        {
            ...
            case ManifoldType.Circles:
                {
                    Vec2 pointA = MathUtils.Multiply(ref xfA, manifold._localPoint);
                    Vec2 pointB = MathUtils.Multiply(ref xfB, manifold._points[0].LocalPoint);
                    _normal = new Vec2(1.0f, 0.0f);
                    if (Vec2.DistanceSquared(pointA, pointB) > Settings.b2_epsilon * Settings.b2_epsilon)
                    {
                        _normal = pointB - pointA;
                        _normal.Normalize();
                    }

                    Vec2 cA = pointA + radiusA * _normal;
                    Vec2 cB = pointB - radiusB * _normal;
            ...
            case ManifoldType.FaceA:
                {
                    ...
                    Vec2 planePoint = MathUtils.Multiply(ref xfA, manifold._localPoint);

                    for (int i = 0; i < manifold._pointCount; ++i)
                    {
                        Vec2 clipPoint = MathUtils.Multiply(ref xfB, manifold._points[i].LocalPoint);
                        Vec2 cA = clipPoint + (radiusA - Vec2.Dot(clipPoint - planePoint, _normal)) * _normal;
                        Vec2 cB = clipPoint - radiusB * _normal;
            ...
            case ManifoldType.FaceB:
                {
                    ...
                    Vec2 planePoint = MathUtils.Multiply(ref xfB, manifold._localPoint);

                    for (int i = 0; i < manifold._pointCount; ++i)
                    {
                        Vec2 clipPoint = MathUtils.Multiply(ref xfA, manifold._points[i].LocalPoint);
                        Vec2 cA = clipPoint - radiusA * _normal;
                        Vec2 cB = clipPoint + (radiusB - Vec2.Dot(clipPoint - planePoint, _normal)) * _normal;
                        _points[i] = 0.5f * (cA + cB);
                    }
        ...
        public Vec2 _normal;                        ///< world vector pointing from A to B
        public FixedArray2<Vec2> _points;       ///< world contact point (point of intersection)
    };
    ...
    public static class Collision
    {
                              ...
                              CircleShape circleB, ref Transform xfB)
        {
            ...
            Vec2 pA = MathUtils.Multiply(ref xfA, circleA._p);
            Vec2 pB = MathUtils.Multiply(ref xfB, circleB._p);
            ...
            manifold._localNormal = Vec2.Zero;
                                       ...
                                       CircleShape circleB, ref Transform xfB)
        {
            ...
            Vec2 c = MathUtils.Multiply(ref xfB, circleB._p);
            Vec2 cLocal = MathUtils.MultiplyT(ref xfA, c);
            ...
            {
                float s = Vec2.Dot(polygonA._normals[i], cLocal - polygonA._vertices[i]);
            ...
            Vec2 v1 = polygonA._vertices[vertIndex1];
            Vec2 v2 = polygonA._vertices[vertIndex2];
            ...
            else
            {
                ...
                float separation2 = Vec2.Dot(cLocal - faceCenter, polygonA._normals[vertIndex1]);
                               ...
                               PolygonShape polyB, ref Transform xfB)
        {
            ...
            Vec2 v11 = poly1._vertices[iv1];
            Vec2 v12 = poly1._vertices[iv2];

            Vec2 localTangent = v12 - v11;
            localTangent.Normalize();

            Vec2 localNormal = MathUtils.Cross(localTangent, 1.0f);
            Vec2 planePoint = 0.5f * (v11 + v12);

            Vec2 tangent = MathUtils.Multiply(ref xf1.R, localTangent);
            Vec2 normal = MathUtils.Cross(tangent, 1.0f);
                               ...
                               CircleShape circleB, ref Transform xfB)
        {
            ...
            Vec2 Q = MathUtils.MultiplyT(ref xfA, MathUtils.Multiply(ref xfB, circleB._p));

            Vec2 A = edgeA._vertex1, B = edgeA._vertex2;
            Vec2 e = B - A;

            // Barycentric coordinates
            float u = Vec2.Dot(e, B - Q);
            float v = Vec2.Dot(e, Q - A);
            ...
            if (v <= 0.0f)
            {
                ...
                {
                    Vec2 A1 = edgeA._vertex0;
                    Vec2 B1 = A;
                    Vec2 e1 = B1 - A1;
                    float u1 = Vec2.Dot(e1, B1 - Q);
            ...
            if (u <= 0.0f)
            {
                ...
                {
                    Vec2 B2 = edgeA._vertex3;
                    Vec2 A2 = B;
                    Vec2 e2 = B2 - A2;
                    float v2 = Vec2.Dot(e2, Q - A2);
        ...
        static EPAxis ComputeEdgeSeperation(Vec2 v1, Vec2 v2, Vec2 n, PolygonShape polygonB, float radius)
        {
            // EdgeA separation
            EPAxis axis;
            axis.type = EPAxisType.EdgeA;
            axis.index = 0;
            axis.separation = Vec2.Dot(n, polygonB._vertices[0] - v1);
            for (int i = 1; i < polygonB._vertexCount; ++i)
            {
                float s = Vec2.Dot(n, polygonB._vertices[i] - v1);
        ...
        static EPAxis ComputePolygonSeperation(Vec2 v1, Vec2 v2, Vec2 n, PolygonShape polygonB, float radius)
        {
            // PolygonB separation
            EPAxis axis;
            ...
            {
                float s1 = Vec2.Dot(polygonB._normals[i], v1 - polygonB._vertices[i]);    
                float s2 = Vec2.Dot(polygonB._normals[i], v2 - polygonB._vertices[i]);
        ...
        static void FindIncidentEdge(ref FixedArray2<ClipVertex> c, PolygonShape poly1, int edge1, PolygonShape poly2)
        {
            ...
            Vec2 normal1 = poly1._normals[edge1];
            ...
            {
                float dot = Vec2.Dot(normal1, poly2._normals[i]);
                if (dot < minDot)
                {
                    minDot = dot;
                    index = i;
                               ...
                               PolygonShape polygonB_in, ref Transform xfB)
        {
            ...
            Vec2 e = v2 - v1;
            Vec2 edgeNormal = new Vec2(e.Y, -e.X);
            edgeNormal.Normalize();

            // Determine side
            bool isFrontSide = Vec2.Dot(edgeNormal, s_polygonB._centroid - v1) >= 0.0f;
            ...
            if (edgeA._hasVertex0)
            {
                Vec2 v0 = edgeA._vertex0;
                float s = Vec2.Dot(edgeNormal, v0 - v1);
            ...
            if (edgeA._hasVertex3)
            {
                Vec2 v3 = edgeA._vertex3;
                float s = Vec2.Dot(edgeNormal, v3 - v2);
                if (s > 0.1f * Settings.b2_linearSlop)
                {
            ...
            for (int i = 0; i < Settings.b2_maxManifoldPoints; ++i)
            {
                ...
                separation = Vec2.Dot(normal, clipPoints2[i].v) - frontOffset;
                                    ...
                                    PolygonShape poly2, ref Transform xf2)
        {
...
#if MATH_OVERLOADS
            Vec2 normal1World = MathUtils.Multiply(ref xf1.R, poly1._normals[edge1]);
            Vec2 normal1 = MathUtils.MultiplyT(ref xf2.R, normal1World);
#else
            Vec2 p1n = poly1._normals[edge1];
            Vec2 normal1World = new Vec2(xf1.R.col1.X * p1n.X + xf1.R.col2.X * p1n.Y, xf1.R.col1.Y * p1n.X + xf1.R.col2.Y * p1n.Y);            
            Vec2 normal1 = new Vec2(normal1World.X * xf2.R.col1.X + normal1World.Y * xf2.R.col1.Y, normal1World.X * xf2.R.col2.X + normal1World.Y * xf2.R.col2.Y); 
#endif
            ...
            for (int i = 0; i < count2; ++i)
            {
                ...
                float dot = Vec2.Dot(poly2._vertices[i], normal1);
#else
                Vec2 p2vi = poly2._vertices[i];
...
#if MATH_OVERLOADS
            Vec2 v1 = MathUtils.Multiply(ref xf1, poly1._vertices[edge1]);
            Vec2 v2 = MathUtils.Multiply(ref xf2, poly2._vertices[index]);
#else
            Vec2 p1ve = poly1._vertices[edge1];
            Vec2 p2vi = poly2._vertices[index];
            Vec2 v1 = new Vec2(xf1.Position.X + xf1.R.col1.X * p1ve.X + xf1.R.col2.X * p1ve.Y,
                                     xf1.Position.Y + xf1.R.col1.Y * p1ve.X + xf1.R.col2.Y * p1ve.Y);
            Vec2 v2 = new Vec2(xf2.Position.X + xf2.R.col1.X * p2vi.X + xf2.R.col2.X * p2vi.Y,
...
#else
            Vec2 v2subv1 = new Vec2(v2.X - v1.X, v2.Y - v1.Y);
                                        ...
                                        PolygonShape poly2, ref Transform xf2)
        {
            ...
            Vec2 d = MathUtils.Multiply(ref xf2, poly2._centroid) - MathUtils.Multiply(ref xf1, poly1._centroid);
            Vec2 dLocal1 = MathUtils.MultiplyT(ref xf1.R, d);
            ...
            {
                float dot = Vec2.Dot(poly1._normals[i], dLocal1);
                if (dot > maxDot)
                {
                    maxDot = dot;
                    edge = i;
                                     ...
                                     PolygonShape poly2, ref Transform xf2)
        {
            ...
            // Get the normal of the reference edge in poly2's frame.
            Vec2 normal1 = MathUtils.MultiplyT(ref xf2.R, MathUtils.Multiply(ref xf1.R, poly1._normals[edge1]));
